...
MAIN_DIRECTORY = os.environ['FILES_DIRECTORY']
SUPPORTED_EXTENSIONS = os.environ['SUPPORTED_EXTENSIONS'].split(',')

def get_files(file_type: str):
    if not file_type:
        output_list = [dir_files for _, _, dir_files in os.walk(MAIN_DIRECTORY) if len(dir_files) != 0]
        return jsonify(output_list), 200
    elif file_type in SUPPORTED_EXTENSIONS:
        output_list = os.listdir(f'{MAIN_DIRECTORY}/{file_type}')
        return jsonify(output_list), 200
...
def upload_image():
    ...
    elif os.path.exists(f'{MAIN_DIRECTORY}/{extension_file}/{file_name}'):
        return {'message': 'Arquivo já existente'}, 409

    try:
        path = f'{MAIN_DIRECTORY}/{extension_file}'
        file_name = received_file.filename
...
def download_zip_file():
    ...
    os.system(f"zip -{compression_rate} -r /tmp/imagens.zip {MAIN_DIRECTORY}/{file_type}")
